...
const supportCardOrientation=$('#supportCardOrientation');
const supportCardOrientationLabel=$('#supportCardOrientationLabel');
...
function currentCardOrientation(){
  if(supportCardOrientation){
    if(supportCardOrientation.value==='portrait') return 'portrait';
    if(supportCardOrientation.value==='landscape') return 'landscape';
  }
  return automaticCardOrientation();
}

function syncSupportOrientationLabel(){
  if(!supportOrientationInfo) return;

  const isCard=supportType.value==='card';

  if(supportCardOrientation){
    supportCardOrientation.style.display=isCard ? '' : 'none';
  }
  if(supportCardOrientationLabel){
    supportCardOrientationLabel.style.display=isCard ? '' : 'none';
  }
  ...
  if(!scores){
    supportOrientationInfo.textContent=
      `Orientation carte : ${ori==='portrait' ? 'verticale' : 'horizontale'}.`;
    return;
  }

  const chosen=scores[ori];
  const recommended=scores.best;
  const chosenLabel=ori==='portrait' ? 'verticale' : 'horizontale';
  const recommendedLabel=recommended==='portrait' ? 'verticale' : 'horizontale';
  const retainedPct=Math.round(chosen.retention*100);
  const extensionPct=Math.round(chosen.extension*100);

  if(supportCardOrientation.value==='auto'){
    supportOrientationInfo.textContent=
      `Recommandée : ${recommendedLabel}. Photo conservée ≈ ${retainedPct} %, extension estimée ≈ ${extensionPct} %.`;
  }else{
    supportOrientationInfo.textContent=
      `Orientation forcée : ${chosenLabel}. Photo conservée ≈ ${retainedPct} %, extension estimée ≈ ${extensionPct} %.`+
      (ori!==recommended ? ` Auto recommande ${recommendedLabel}.` : '');
  }
}
...
function refreshSupportOrientationNow(){
  supportRunning=false;
  cancelAnimationFrame(supportRAF);
  supportRAF=0;
  supportStartTime=0;
...
supportCardOrientation.addEventListener('change',refreshSupportOrientationNow);

// Quand une nouvelle photo est chargée, le mode auto recalcule le bon format.
file.addEventListener('change',()=>{
  setTimeout(()=>{
    if(supportCardOrientation.value==='auto'){
      refreshSupportOrientationNow();
    }else{
      syncSupportOrientationLabel();
    }
  },300);
