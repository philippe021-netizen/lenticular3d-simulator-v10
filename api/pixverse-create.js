...
const ALLOWED_MODES = new Set(['standard','transition','omni','multi_transition','modify','mask_selection','mimic']);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!basePrompt && mode !== 'multi_transition' && mode !== 'mask_selection' && mode !== 'mimic') {
    ...
    if (mode === 'mimic') {
      ...
      if (!imgId || (!videoMediaId && !sourceVideoId)) {
        return res.status(400).json({ error: 'Mimic requiert img_id et video_media_id ou source_video_id.' });
      }
      payload = {
        img_id: imgId,
