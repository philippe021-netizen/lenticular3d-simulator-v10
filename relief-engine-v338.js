...
function drawHybridSourceBackdrop(ctx,W,H,dx,dy,dw,dh){
  ...
  // 1) Base plein cadre : même photo, mais légèrement adoucie sur les bords.
  ctx.save();
  ctx.filter='blur(10px) saturate(0.96) brightness(0.94)';
  drawImageCover(ctx,sourceImg,W,H,0,0);
  ctx.restore();

  // 2) Photo originale nette au centre : c'est elle qui porte la qualité visuelle.
...
function drawPreservedReliefToCanvas(norm,target){
  ...
  // Zone utile dans laquelle on garde la photo nette sans couper le sujet.
  ...
  // Sujet détouré au-dessus, aligné sur la photo originale.
