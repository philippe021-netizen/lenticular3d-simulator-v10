...
(() => {
  ...
  // V3.3.5 — sélecteur de zone au stylet, sans rectangle imposé.
  // Apple Pencil / souris : trace libre. Doigt : déplacement. Deux doigts : zoom + déplacement.
  // La sortie conserve x/y/w/h pour rester compatible avec action-preview-engine.js.
  let zoneView={zoom:1,panX:0,panY:0};
  let zonePoints=[];
  let zonePointers=new Map();
  let zoneDrawPointer=null;
  let zonePanPointer=null;
  let zoneLastPan=null;
  let zonePinch=null;
  let zoneLoadedRect=null;

  function zoneCanvasPos(e){
    const r=zoneCanvas.getBoundingClientRect();
    return {x:(e.clientX-r.left)*zoneCanvas.width/Math.max(1,r.width),y:(e.clientY-r.top)*zoneCanvas.height/Math.max(1,r.height)};
  }
  function zoneImageSize(){
    const im=src();
    return {w:im?.naturalWidth||im?.width||1,h:im?.naturalHeight||im?.height||1};
  }
  function zoneScreenToImage(p){
    const im=src(); if(!im) return {x:0,y:0};
    const f=fit(im,zoneCanvas.width,zoneCanvas.height);
    const cx=zoneCanvas.width/2,cy=zoneCanvas.height/2;
    const bx=(p.x-cx-zoneView.panX)/zoneView.zoom+cx;
    const by=(p.y-cy-zoneView.panY)/zoneView.zoom+cy;
    return {
      x:(bx-f.x)/Math.max(.0001,f.w)*(im.naturalWidth||im.width||1),
      y:(by-f.y)/Math.max(.0001,f.h)*(im.naturalHeight||im.height||1)
    };
  }
  function zoneImageToScreen(q){
    const im=src(); if(!im) return {x:0,y:0};
    const f=fit(im,zoneCanvas.width,zoneCanvas.height);
    const iw=im.naturalWidth||im.width||1,ih=im.naturalHeight||im.height||1;
    let x=f.x+(q.x/iw)*f.w,y=f.y+(q.y/ih)*f.h;
    const cx=zoneCanvas.width/2,cy=zoneCanvas.height/2;
    x=cx+(x-cx)*zoneView.zoom+zoneView.panX;
    y=cy+(y-cy)*zoneView.zoom+zoneView.panY;
    return {x,y};
  }
  function zoneRectFromPoints(points){
    if(!Array.isArray(points)||points.length<3) return null;
    const {w,h}=zoneImageSize();
    ...
    minX=clamp(minX,0,w);minY=clamp(minY,0,h);maxX=clamp(maxX,0,w);maxY=clamp(maxY,0,h);
    if(maxX-minX<4||maxY-minY<4) return null;
    return {x:minX/w,y:minY/h,w:(maxX-minX)/w,h:(maxY-minY)/h};
  }
  function zonePointsFromRect(z){
    if(!z) return [];
    const {w,h}=zoneImageSize();
    const x=z.x*w,y=z.y*h,rw=z.w*w,rh=z.h*h;
    // Le rectangle existant est affiché comme un contour fermé modifiable ;
    // le prochain tracé au stylet le remplace entièrement.
    return [{x,y},{x:x+rw,y},{x:x+rw,y:y+rh},{x,y:y+rh},{x,y}];
  }
  function resetZoneView(){zoneView={zoom:1,panX:0,panY:0};zonePointers.clear();zonePinch=null;zonePanPointer=null;zoneLastPan=null;}
  ...
  function ensureZoneUI(){
    ...
    cancel.onclick=()=>finishZone(null,true);
    ok.onclick=()=>finishZone(zoneRectFromPoints(zonePoints),false);
    top.append(title,cancel,ok);

    const body=document.createElement('div');Object.assign(body.style,{flex:'1',minHeight:'0',display:'flex',alignItems:'center',justifyContent:'center',padding:'12px',overflow:'hidden'});
    ...
    const foot=document.createElement('div');foot.textContent='Apple Pencil : entoure précisément la zone. Doigt : déplacer. 2 doigts : zoomer/déplacer.';Object.assign(foot.style,{padding:'10px',textAlign:'center',fontSize:'12px',opacity:.8});
    zoneModal.append(top,body,foot);document.body.appendChild(zoneModal);

    zoneCanvas.addEventListener('pointerdown',e=>{
      e.preventDefault();
      const p=zoneCanvasPos(e);
      zonePointers.set(e.pointerId,p);
      zoneCanvas.setPointerCapture?.(e.pointerId);

      // Le Pencil (ou la souris en test desktop) dessine. Le doigt sert à naviguer.
      const draws=e.pointerType==='pen'||e.pointerType==='mouse';
      if(draws && zonePointers.size===1){
        zoneDrawPointer=e.pointerId;
        zonePanPointer=null;zoneLastPan=null;zonePinch=null;
        zonePoints=[zoneScreenToImage(p)];
        zoneLoadedRect=null;
        drawZone();
        return;
      }
      zoneDrawPointer=null;
      if(zonePointers.size>=2){
        const a=[...zonePointers.values()].slice(0,2);
        const mid={x:(a[0].x+a[1].x)/2,y:(a[0].y+a[1].y)/2};
        zonePinch={dist:Math.max(1,Math.hypot(a[1].x-a[0].x,a[1].y-a[0].y)),zoom:zoneView.zoom,panX:zoneView.panX,panY:zoneView.panY,mid};
        zonePanPointer=null;zoneLastPan=null;
      }else{
        zonePanPointer=e.pointerId;zoneLastPan=p;
      }
    },{passive:false});

    zoneCanvas.addEventListener('pointermove',e=>{
      if(!zonePointers.has(e.pointerId))return;
      e.preventDefault();
      const events=typeof e.getCoalescedEvents==='function'?e.getCoalescedEvents():[e];
      for(const ev of events){
        const p=zoneCanvasPos(ev);zonePointers.set(e.pointerId,p);
        if(zoneDrawPointer===e.pointerId){
          const q=zoneScreenToImage(p),last=zonePoints[zonePoints.length-1];
          if(!last||Math.hypot(q.x-last.x,q.y-last.y)>1.2)zonePoints.push(q);
          continue;
        }
        if(zonePointers.size>=2&&zonePinch){
          const a=[...zonePointers.values()].slice(0,2);
          const d=Math.max(1,Math.hypot(a[1].x-a[0].x,a[1].y-a[0].y));
          const mid={x:(a[0].x+a[1].x)/2,y:(a[0].y+a[1].y)/2};
          const newZoom=clamp(zonePinch.zoom*d/zonePinch.dist,1,12);
          // conserve le point image sous le milieu initial pendant le pinch
          const oldZoom=zoneView.zoom,oldPanX=zoneView.panX,oldPanY=zoneView.panY;
          zoneView.zoom=zonePinch.zoom;zoneView.panX=zonePinch.panX;zoneView.panY=zonePinch.panY;
          const anchor=zoneScreenToImage(zonePinch.mid);
          zoneView.zoom=newZoom;
          const anchorScreen=zoneImageToScreen(anchor);
          zoneView.panX+=mid.x-anchorScreen.x;zoneView.panY+=mid.y-anchorScreen.y;
          if(!Number.isFinite(zoneView.panX)||!Number.isFinite(zoneView.panY)){zoneView.zoom=oldZoom;zoneView.panX=oldPanX;zoneView.panY=oldPanY;}
        }else if(zonePanPointer===e.pointerId&&zoneLastPan){
          zoneView.panX+=p.x-zoneLastPan.x;zoneView.panY+=p.y-zoneLastPan.y;zoneLastPan=p;
        }
      }
      drawZone();
    },{passive:false});
    ...
    const end=e=>{
      if(zoneDrawPointer===e.pointerId){
        ...
        if(zonePoints.length>2){
          ...
          if(Math.hypot(first.x-last.x,first.y-last.y)>1)zonePoints.push({...first});
        }
      }
      zonePointers.delete(e.pointerId);
      if(zonePanPointer===e.pointerId){zonePanPointer=null;zoneLastPan=null;}
      if(zonePointers.size<2)zonePinch=null;
      if(zonePointers.size===1&&!zoneDrawPointer){
        const [id,p]=[...zonePointers.entries()][0];zonePanPointer=id;zoneLastPan=p;
      }
      drawZone();
    };
    zoneCanvas.addEventListener('pointerup',end);zoneCanvas.addEventListener('pointercancel',end);
  }
  function drawZone(){
    const im=src();if(!im||!zoneCtx)return;
    zoneCtx.setTransform(1,0,0,1,0,0);zoneCtx.clearRect(0,0,zoneCanvas.width,zoneCanvas.height);
    const f=fit(im,zoneCanvas.width,zoneCanvas.height),cx=zoneCanvas.width/2,cy=zoneCanvas.height/2;
    zoneCtx.save();zoneCtx.translate(cx+zoneView.panX,cy+zoneView.panY);zoneCtx.scale(zoneView.zoom,zoneView.zoom);zoneCtx.translate(-cx,-cy);zoneCtx.drawImage(im,f.x,f.y,f.w,f.h);zoneCtx.restore();

    if(zonePoints.length){
      zoneCtx.save();zoneCtx.beginPath();
      zonePoints.forEach((q,i)=>{const p=zoneImageToScreen(q);if(i===0)zoneCtx.moveTo(p.x,p.y);else zoneCtx.lineTo(p.x,p.y);});
      if(!zoneDrawPointer&&zonePoints.length>2)zoneCtx.closePath();
      zoneCtx.lineWidth=Math.max(2,zoneCanvas.width/340);zoneCtx.strokeStyle='#00e5ff';zoneCtx.setLineDash([]);zoneCtx.stroke();
      if(!zoneDrawPointer&&zonePoints.length>2){zoneCtx.fillStyle='rgba(0,229,255,.13)';zoneCtx.fill();}
      zoneCtx.restore();
    }

    // Pointeur central permanent : aide au décalage précis quand on zoome.
    zoneCtx.save();zoneCtx.strokeStyle='rgba(255,255,255,.82)';zoneCtx.lineWidth=1.5;
    zoneCtx.beginPath();zoneCtx.moveTo(cx-10,cy);zoneCtx.lineTo(cx+10,cy);zoneCtx.moveTo(cx,cy-10);zoneCtx.lineTo(cx,cy+10);zoneCtx.stroke();zoneCtx.restore();
  }
  function finishZone(rect,cancelled=false){
    zoneModal.style.display='none';
    const cb=zoneResolve;zoneResolve=null;
    const fallback=zoneLoadedRect;
    const res=cancelled?null:(rect||fallback||null);
    zonePoints=[];zoneLoadedRect=null;zonePointers.clear();zoneDrawPointer=null;zonePanPointer=null;zoneLastPan=null;zonePinch=null;
    cb?.(res);
  }
  function chooseZone(s,label){
    ensureZoneUI();const im=src();if(!im)return Promise.resolve(null);
    const max=1400,sw=im.naturalWidth||im.width,sh=im.naturalHeight||im.height,sc=Math.min(1,max/Math.max(sw,sh));
    zoneCanvas.width=Math.max(2,Math.round(sw*sc));zoneCanvas.height=Math.max(2,Math.round(sh*sc));
    zoneModal._title.textContent=label;zoneModal.style.display='flex';resetZoneView();
    zoneLoadedRect=s.actionZone||null;zonePoints=zonePointsFromRect(zoneLoadedRect);drawZone();
    return new Promise(r=>zoneResolve=r);
  }
