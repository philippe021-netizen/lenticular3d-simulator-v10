...
tabs.forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    if(target==='support') setTimeout(()=>{ syncSupportOptions(); if(supportCardOrientation.value==='auto') refreshSupportOrientationNow(); },0);
  });
});
...
supportChoiceButtons.forEach(btn=>{
  btn.addEventListener('click',()=>chooseSupport(btn.dataset.support));
});
...
supportType.addEventListener('change',()=>{
  ...
  if(supportType.value==='keychain-vertical'){
    supportStatus.textContent='Porte-clé vertical prêt pour l’aperçu.';
  }else if(supportType.value==='keychain-horizontal'){
    supportStatus.textContent='Porte-clé horizontal prêt pour l’aperçu.';
  }else{
