...
async function prepareWithOutpaint(){
  ...
  const targetRatio=prepareTarget.value==='square' ? 1 : (85.6/54);
  ...
  try{
    ...
    prepareStatus.textContent=
      'Photo étendue au format cible. Vérifie le décor, puis clique sur « Créer le relief 3D local ».';
...
function drawPreservedReliefToCanvas(norm,target){
  if(!subjectImg || !sourceImg) return false;
  ...
  // If the image was already prepared by Outpaint, NEVER extend it again.
  if(aiPrepared){
    ...
    const dw=iw*scale;
    const dh=ih*scale;
    const dx=(W-dw)/2 + posX*(W*0.30);
    const dy=(H-dh)/2 + posY*(H*0.30);

    // Clean prepared background, exactly as returned by Outpaint.
    ctx.fillStyle='#111';
    ctx.fillRect(0,0,W,H);
    ctx.drawImage(sourceImg,dx,dy,dw,dh);

    // Subject keeps only the lenticular parallax.
  ...
  // Legacy local preserve mode for non-AI-prepared photos.
  const safeW=W*(1-margin*2);
  const safeH=H*(1-margin*2);

  const baseScale=Math.min(safeW/iw,safeH/ih);
  const scale=baseScale*zoom;
  ...
  drawHybridSourceBackdrop(ctx,W,H,dx,dy,dw,dh);
