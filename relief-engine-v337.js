...
function drawPreservedReliefToCanvas(norm,target){
  ...
  // 1) Fond : plein cadre. On garde seulement une parallaxe très faible.
  const bgShift=norm*5*(Number(bgDepth.value)/0.10);
  drawImageCover(ctx,backgroundImg,W,H,bgShift,0);

  // 2) Sujet : conserve son ratio et reste entièrement visible.
  const iw=subjectImg.naturalWidth;
  const ih=subjectImg.naturalHeight;

  // Safe box inside the support.
  ...
  // Main 3D translation remains horizontal, but much smaller than the old crop motion.
  const subShift=norm*14*(Number(subjectDepth.value)/0.30);

  const dx=(W-dw)/2 + posX*(W*0.35) + subShift;
