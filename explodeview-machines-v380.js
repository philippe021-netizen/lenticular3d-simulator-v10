/* HappyHolo V3.8.3 — ExplodeView générique pour machines et objets techniques */
(() => {
  'use strict';
  ...
  const explodeSelections=()=>plan().filter((s,i)=>i>0&&s?.action==='explodeview');
  ...
  function loadSlimSAM(){
    ...
    slimSAMLoading=new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script');s.src='./slimsam-piece-selector-v330.js?v=331';s.async=true;s.dataset.happyholoSlimsam='1';
  ...
  function statusText(){
    ...
    if(!all?.[0]?.mask)return 'Crée d’abord le relief global de la machine.';
    if(!pieces.length)return 'Relief prêt. Touche « IA — toucher une pièce » puis sélectionne roue, selle, phare, moteur…';
  ...
  function notify(){window.dispatchEvent(new CustomEvent('happyholo-action-plan-changed'));window.dispatchEvent(new CustomEvent('happyholo-background-changed'));try{window.renderAt?.(0,window.HappyHoloReliefState?.view);}catch(_){}updatePanel();}

  function configureAutomatically(){
    const selections=plan();
    const ranked=selections.slice(1).map((s,j)=>({s,i:j+1,stats:maskStats(s.mask)})).filter(x=>x.stats).sort((a,b)=>a.stats.area-b.stats.area);
    if(ranked.length<2){alert('Sélectionne au moins deux grosses pièces avec « IA — toucher une pièce » avant de préparer ExplodeView.');return;}
    if(selections[0]){selections[0].action='none';delete selections[0].explodeOrder;delete selections[0].explodeDirection;delete selections[0].explodeMode;}
    ranked.forEach((item,rank)=>{item.s.action='explodeview';item.s.explodeOrder=rank+1;item.s.explodeMode=state.mode;item.s.explodeDirection='auto';if(!Number.isFinite(Number(item.s.intensity))||Number(item.s.intensity)===50)item.s.intensity=65;});
    window.dispatchEvent(new CustomEvent('happyholo:selection-plan',{detail:{count:selections.length,source:'explodeview-ai-parts'}}));setTimeout(notify,30);
  }
  function disable(){for(const s of plan())if(s.action==='explodeview'){s.action='none';delete s.explodeOrder;delete s.explodeDirection;delete s.explodeMode;}window.dispatchEvent(new CustomEvent('happyholo:selection-plan',{detail:{count:plan().length,source:'explodeview-disable'}}));setTimeout(notify,30);}

  let panel=null,status=null,modeSelect=null;
  function ensurePanel(){
    const anchor=document.getElementById('happyHoloSelectionControls');if(!anchor||!plan().length)return;
    panel=document.getElementById('happyHoloExplodeViewControls');
    if(!panel){
      panel=document.createElement('section');panel.id='happyHoloExplodeViewControls';panel.style.cssText='background:linear-gradient(145deg,#071018,#102536);color:#fff;border:2px solid #28a8ee;border-radius:18px;padding:16px;margin:16px 0;box-shadow:0 12px 32px #00111f26';
      ...
      const aiInfo=document.createElement('div');aiInfo.textContent='Touche une roue, un phare, une selle ou un grand élément. SlimSAM calcule le masque complet. Recommence pour chaque pièce.';aiInfo.style.cssText='margin-top:7px;color:#c9dce9;font-size:11px;line-height:1.45';
      aiBtn.addEventListener('click',async()=>{const old=aiBtn.textContent;aiBtn.disabled=true;aiBtn.textContent='Chargement du sélecteur IA…';try{const api=await loadSlimSAM();aiBtn.textContent=old;await api.open();}catch(e){console.error('[SlimSAM loader]',e);alert('Sélection IA indisponible : '+(e?.message||e));}finally{aiBtn.disabled=false;aiBtn.textContent=old;}});panel.append(aiBtn,aiInfo);
      ...
      const note=document.createElement('div');note.style.cssText='margin-top:8px;color:#9eb0bf;font-size:11px;line-height:1.45';note.textContent='Le masque global reste fixe. Seules les pièces IA sont déplacées dans l’ExplodeView.';panel.appendChild(note);
      modeSelect.addEventListener('change',()=>{state.mode=modeSelect.value;try{localStorage.setItem('happyholo:explodeview-mode',state.mode);}catch(_){}for(const s of explodeSelections())s.explodeMode=state.mode;notify();});prepare.addEventListener('click',configureAutomatically);stop.addEventListener('click',disable);anchor.insertAdjacentElement('afterend',panel);
    ...
    }else if(anchor.nextElementSibling!==panel)anchor.insertAdjacentElement('afterend',panel);updatePanel();
  }
  function updatePanel(){if(!panel)return ensurePanel();if(modeSelect)modeSelect.value=state.mode;if(status)status.textContent=statusText();}

  async function openAISelector(statusBox){
    try{if(statusBox)statusBox.textContent='Chargement de la sélection IA par toucher…';const api=await loadSlimSAM();await api.open();if(statusBox)statusBox.textContent='Sélection IA ouverte. Touche les grosses pièces une par une.';}
    catch(e){console.error('[SlimSAM main selector]',e);if(statusBox)statusBox.textContent='Sélection IA indisponible : '+(e?.message||e);alert('Sélection IA indisponible : '+(e?.message||e));}
  }

  function setupMainSelectionButton(){
    const start=document.getElementById('startPieceSelection'),file=document.getElementById('file'),build=document.getElementById('build'),statusBox=document.getElementById('status'),autoEnabled=document.getElementById('autoPieceSelectionEnabled');
    if(!start||!file||!build||start.dataset.ready==='1')return;start.dataset.ready='1';
    if(autoEnabled){autoEnabled.checked=false;autoEnabled.disabled=true;const label=autoEnabled.closest('label');if(label)label.style.display='none';}
    build.addEventListener('click',()=>{window.HappyHoloPendingAutoPieceSplit=false;},true);
    start.addEventListener('click',async()=>{
      if(!file.files?.length){window.HappyHoloPendingAutoPieceSplit=false;if(statusBox)statusBox.textContent='Choisis d’abord la photo de la machine.';file.scrollIntoView({block:'center',behavior:'smooth'});file.click();return;}
      window.HappyHoloPendingAutoPieceSplit=false;
      if(plan()?.[0]?.mask){await openAISelector(statusBox);return;}
      if(statusBox)statusBox.textContent='Création du détourage global, puis ouverture automatique de la sélection IA…';
      let opened=false;const tryOpen=async()=>{if(opened||!plan()?.[0]?.mask)return;opened=true;window.removeEventListener('happyholo-relief-ready',tryOpen);window.removeEventListener('happyholo:selection-plan',tryOpen);await openAISelector(statusBox);};
      window.addEventListener('happyholo-relief-ready',tryOpen);window.addEventListener('happyholo:selection-plan',tryOpen);build.click();setTimeout(tryOpen,1200);
    });
    const sync=()=>{start.disabled=build.disabled&&!plan()?.[0]?.mask;start.textContent=build.disabled&&!plan()?.[0]?.mask?'Création du détourage…':'🎯 IA — toucher les pièces';};new MutationObserver(sync).observe(build,{attributes:true,attributeFilter:['disabled']});sync();
  }
  ...
  console.log('[HAPPYHOLO] ExplodeView machines V3.8.3 + SlimSAM main selector actif');
})();