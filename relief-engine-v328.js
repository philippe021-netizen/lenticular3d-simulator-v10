...
async function happyHoloWarmOfflineEngines(progress){
  const tell=t=>{ try{progress?.(t);}catch{} };
  tell('Préparation du détourage local…');
  const c=document.createElement('canvas'); c.width=96;c.height=96;
  const cx=c.getContext('2d');
  cx.fillStyle='#f2f2f2';cx.fillRect(0,0,96,96);
  cx.fillStyle='#222';cx.beginPath();cx.arc(48,48,25,0,Math.PI*2);cx.fill();
  const dummy=await new Promise((resolve,reject)=>c.toBlob(b=>b?resolve(b):reject(new Error('Image test impossible')),'image/png'));
  const f=new File([dummy],'happyholo-offline-test.png',{type:'image/png'});
  const cut=await localRemoveBackground(f);
  if(!cut) throw new Error('Le modèle de détourage n’a pas pu être préparé.');
  tell('Préparation du moteur de profondeur…');
  const est=await getEstimator();
  if(!est) throw new Error('Le modèle de profondeur n’a pas pu être préparé.');
  tell('Moteurs IA locaux préparés.');
  return true;
}
window.happyHoloWarmOfflineEngines=happyHoloWarmOfflineEngines;
...
function preparedState(){
  const p=window.HappyHoloPreparedBackground;
  return p && p.ready && p.image ? p : null;
}
function drawPreparedBase(ctx,p,W,H,norm,amplitude,bgK){
  const f=fitCover(p.image,W,H);
  const shift=norm*5*amplitude*bgK;
  ctx.drawImage(p.image,f.x+shift,f.y,f.w,f.h);
}
function drawPreparedLayers(ctx,p,W,H,norm,amplitude,phase){
  if(!p)return;
  const f=fitCover(p.image,W,H);
  for(const l of p.layers||[]){
    if(l.phase!==phase || !l.canvas)continue;
    const d=Number(l.depth)||0;
    const shift=norm*12*amplitude*(d/0.30);
    ctx.drawImage(l.canvas,f.x+shift,f.y,f.w,f.h);
  }
}
...
function renderAt(norm,target=view){
  ...
  if(prepared){
    drawPreparedBase(x,prepared,W,H,norm,amplitude,bgK);
    drawPreparedLayers(x,prepared,W,H,norm,amplitude,'behind');
  }else{
  ...
  let tmp;
  if(window.HappyHoloActionLocal?.renderSubject){
    tmp=window.HappyHoloActionLocal.renderSubject(subjectImg,W,H,norm);
  }else{
    tmp=document.createElement('canvas'); tmp.width=W;tmp.height=H;
    const tx0=tmp.getContext('2d');
    const fs0=fitCover(subjectImg,W,H); tx0.drawImage(subjectImg,fs0.x,fs0.y,fs0.w,fs0.h);
  }
  ...
  if(prepared) drawPreparedLayers(x,prepared,W,H,norm,amplitude,'front');
}
...
buildBtn.addEventListener('click',async()=>{
  ...
  try{
    ...
    window.HappyHoloReliefState={sourceImg,subjectImg,backgroundImg,subjectDepthCanvas,backgroundDepthCanvas,view,preparedBackground:window.HappyHoloPreparedBackground||null};
...
exportBtn.addEventListener('click',async()=>{
  ...
  downloadBtn.disabled=false; startPreview(); setStatus('9 vues V3.27 prêtes.');
});

downloadBtn.addEventListener('click',async()=>{
  if(exported.length!==9) return;
  if(!window.HappyHoloZipLocal?.createZip){
    setStatus('ZIP local indisponible.');
    return;
  }
  setStatus('Création du ZIP local…');
  const entries=exported.map((b,i)=>({name:`vue-${String(i+1).padStart(2,'0')}.png`,data:b}));
  entries.push({name:'manifest.json',data:JSON.stringify({
    generator:'HappyHolo Relief 3D V3.28 offline iPad',localSegmentation:true,externalPaidApi:false,views:9,
  ...
  const b=await window.HappyHoloZipLocal.createZip(entries); const u=URL.createObjectURL(b);
  const a=document.createElement('a'); a.href=u; a.download='9-vues-happyholo-v328.zip'; a.click();
  setTimeout(()=>URL.revokeObjectURL(u),2000);
  setStatus('ZIP 9 vues créé localement.');
});