/* HappyHolo V3.32.1 — MediaPipe MagicTouch interactive machine part selector */
(() => {
  'use strict';
  ...
  async function getApi(){ apiPromise ||= import(LIB); return apiPromise; }
  async function getSegmenter(setStatus){
    if(!segmenterPromise){
      segmenterPromise=(async()=>{
        ...
        return api.InteractiveSegmenter.createFromOptions(vision,{baseOptions:{modelAssetPath:MODEL,delegate:'CPU'}});
      })();
    }
  ...
  function maskToImageData(mask,targetW,targetH,baseMask,threshold){
    const srcW=mask.width,srcH=mask.height,values=mask.getAsFloat32Array();
    const src=document.createElement('canvas');src.width=srcW;src.height=srcH;
    const sctx=src.getContext('2d',{willReadFrequently:true}),out=sctx.createImageData(srcW,srcH),t=Number(threshold)||0.45;
    for(let i=0;i<srcW*srcH;i++){
      const a=values[i]>=t?Math.min(255,Math.max(0,Math.round(values[i]*255))):0,o=i*4;
      out.data[o]=0;out.data[o+1]=200;out.data[o+2]=255;out.data[o+3]=a;
    }
    ...
    if(baseMask?.data){for(let i=0;i<targetW*targetH;i++){const o=i*4;converted.data[o+3]=Math.min(converted.data[o+3],baseMask.data[o+3]||0);}}
    return converted;
  }
  ...
  function addSelection(mask){
    ...
    selections.push({id:Date.now()+Math.random(),name:`Pièce IA ${existing+1}`,depth:.34+existing*.025,action:'explodeview',intensity:65,timing:'all',actionZones:[],explodeOrder:existing+1,explodeDirection:'auto',explodeMode:mode,source:'mediapipe-magic-touch',confidence:1,mask,initialMask:cloneImageData(mask)});
  ...
  async function open(){
    const selections=plan(),file=document.getElementById('file')?.files?.[0];
    ...
    const base=selections[0].mask;

    const overlay=document.createElement('div');overlay.style.cssText='position:fixed;inset:0;z-index:10000150;background:#05080bf5;color:#fff;display:flex;flex-direction:column;padding:max(10px,env(safe-area-inset-top)) 10px max(10px,env(safe-area-inset-bottom));box-sizing:border-box;font-family:system-ui,sans-serif';
    ...
    const title=document.createElement('div');title.innerHTML='<b style="font-size:19px">Magic Touch — sélection d’une pièce</b><div style="font-size:12px;color:#c7d7e3;margin-top:3px">Vert = ajouter · Rouge = retirer · puis Valider la pièce.</div>';
    ...
    const plus=mk('➕ Ajouter','#76e59a'),minus=mk('➖ Retirer','#ff8f8f'),reset=mk('↺ Refaire pièce','#ddd'),validate=mk('✓ Valider la pièce','#62c7ff');controls.append(plus,minus,reset,validate);
    const thresholdWrap=document.createElement('label');thresholdWrap.style.cssText='display:flex;align-items:center;gap:8px;font-size:12px;margin:0 0 8px;color:#d9e5ed';thresholdWrap.innerHTML='Précision du bord <input type="range" min="25" max="75" value="45" step="5" style="flex:1"><b>45%</b>';
    const threshold=thresholdWrap.querySelector('input'),thresholdOut=thresholdWrap.querySelector('b');
    const status=document.createElement('div');status.textContent='Chargement de la photo…';status.style.cssText='padding:9px 11px;border-radius:10px;background:#142531;color:#dce9f2;font-size:12px;margin-bottom:8px';
    ...
    const sourceCanvas=document.createElement('canvas'),maskCanvas=document.createElement('canvas'),strokeCanvas=document.createElement('canvas');
    for(const c of [sourceCanvas,maskCanvas,strokeCanvas]) c.style.cssText='position:absolute;pointer-events:none;display:block';
    maskCanvas.style.opacity='.62';stage.append(sourceCanvas,maskCanvas,strokeCanvas);overlay.append(head,controls,thresholdWrap,status,stage);document.body.appendChild(overlay);
    ...
    const img=new Image();img.decoding='async';img.src=await fileToDataURL(file);
    await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;});
    sourceCanvas.width=maskCanvas.width=strokeCanvas.width=base.width;
    sourceCanvas.height=maskCanvas.height=strokeCanvas.height=base.height;
    const sourceCtx=sourceCanvas.getContext('2d',{alpha:false});sourceCtx.fillStyle='#111';sourceCtx.fillRect(0,0,base.width,base.height);sourceCtx.drawImage(img,0,0,base.width,base.height);

    function syncLayout(){
      const r=stage.getBoundingClientRect(),aspect=base.width/base.height;
      let dw=r.width,dh=dw/aspect;if(dh>r.height){dh=r.height;dw=dh*aspect;}
      const left=(r.width-dw)/2,top=(r.height-dh)/2;
      for(const c of [sourceCanvas,maskCanvas,strokeCanvas]){c.style.left=left+'px';c.style.top=top+'px';c.style.width=dw+'px';c.style.height=dh+'px';}
    }
    syncLayout();window.addEventListener('resize',syncLayout,{passive:true});
    ...
    try{
      ...
      segmenter.setImage(sourceCanvas);activeImageKey=key;
      status.textContent='Photo affichée. Mode AJOUTER : touche la roue, selle, phare, moteur…';
    ...
    function containPoint(ev){
      const r=sourceCanvas.getBoundingClientRect(),px=ev.clientX-r.left,py=ev.clientY-r.top;
    ...
    function drawStrokes(){
      ...
      for(const s of all){if(!s.point?.length)continue;c.save();c.strokeStyle=s.brushMode==='negative'?'#ff3b30':'#34c759';c.fillStyle=c.strokeStyle;c.lineWidth=Math.max(4,base.width/180);c.lineCap='round';c.lineJoin='round';if(s.point.length===1){const p=s.point[0];c.beginPath();c.arc(p.x*c.canvas.width,p.y*c.canvas.height,Math.max(7,base.width/140),0,Math.PI*2);c.fill();c.strokeStyle='#fff';c.lineWidth=2;c.stroke();}else{c.beginPath();c.moveTo(s.point[0].x*c.canvas.width,s.point[0].y*c.canvas.height);for(let i=1;i<s.point.length;i++)c.lineTo(s.point[i].x*c.canvas.width,s.point[i].y*c.canvas.height);c.stroke();}c.restore();}
    }
    async function recompute(){
      if(busy||!strokes.length)return;busy=true;validate.disabled=true;
      try{
        status.textContent='Calcul précis des limites de la pièce…';const api=await getApi();
        ...
        const result=segmenter.segment(formatted),mpMask=result?.confidenceMasks?.[0]||result?.categoryMask||result;
    ...
    reset.onclick=()=>{strokes=[];currentPoints=[];currentMask=null;maskCanvas.getContext('2d').clearRect(0,0,base.width,base.height);drawStrokes();setBrush('positive');status.textContent='Pièce effacée. Touche à nouveau une zone à conserver.';};
  ...
  window.HappyHoloMagicTouch={open};window.dispatchEvent(new CustomEvent('happyholo-magictouch-ready'));console.log('[HAPPYHOLO] MediaPipe MagicTouch V3.32.1 chargé');
})();
