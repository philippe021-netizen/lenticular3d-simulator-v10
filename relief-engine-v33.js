...
function drawReliefIntoSupport(norm){
  if(!reliefReady || !subjectImg || !backgroundImg) return;

  // Render the technical relief into an offscreen canvas first.
  ...
  if(type==='card'){
    // cover crop
    const r=temp.width/temp.height;
    const target=W/H;
    let sx=0,sy=0,sw=temp.width,sh=temp.height;

    if(r>target){
      sw=temp.height*target;
      sx=(temp.width-sw)/2;
    }else{
      sh=temp.width/target;
      sy=(temp.height-sh)/2;
    }

    supportCtx.drawImage(temp,sx,sy,sw,sh,0,0,W,H);

    // subtle lens-like sheen
    ...
    const scale=Math.max(W/temp.width,H/temp.height);
...
supportPlay.addEventListener('click',()=>{
  if(!reliefReady){
    supportStatus.textContent=
      'Crée d’abord le relief 3D dans le premier onglet.';
    return;
  }
  ...
  supportStatus.textContent=
    supportType.value==='card'
      ? 'Aperçu carte CB en cours.'
      : 'Aperçu médaillon en cours.';
});
...
function proRefreshList(){
  ...
  proZones.forEach((z,index)=>{
    ...
    sel.addEventListener('change',()=>{
      ...
      proRefreshList();proRefreshOverlay();
    });
...
async function detectText(){
  ...
  for(const line of lines){
    ...
    const type=h>42 || text.length>0 && /^[A-Z0-9][A-Z0-9\s&.-]{3,}$/.test(text) ? 'title' : 'text';
...
proAnalyze.addEventListener('click',async()=>{
  ...
  try{
    ...
    proSetStatus(
      ...
      'Ajoute/corrige le logo manuellement si nécessaire.'
    );
  }catch(e){
    console.error(e);
...
proAddZone.addEventListener('click',()=>{
  if(!proImg){
    proSetStatus('Importe d’abord une carte.');
    return;
  }
  ...
  proSetStatus('Trace un rectangle autour du logo ou de l’élément à mettre en relief.');
});
...
function eventCanvasPoint(e){
  ...
  return {
    x:(clientX-r.left)*proCanvas.width/r.width,
    y:(clientY-r.top)*proCanvas.height/r.height
  };
}
...
function endZone(e){
  ...
  proSetStatus('Zone ajoutée. Ajuste son type et sa profondeur dans la liste.');
}

proCanvas.addEventListener('pointerdown',beginZone);
proCanvas.addEventListener('pointerup',endZone);
proCanvas.addEventListener('touchstart',beginZone,{passive:false});
proCanvas.addEventListener('touchend',endZone,{passive:false});

/* ----- Build base image with selected zones locally filled ----- */
function buildProBase(){
  ...
  const bx=proBaseCanvas.getContext('2d',{willReadFrequently:true});
  ...
  for(const z of proZones){
    ...
    // Sample thin bands around the region and average them.
    let sr=0,sg=0,sb=0,n=0;
    const data=bx.getImageData(0,0,proBaseCanvas.width,proBaseCanvas.height).data;

    const sample=(xx,yy)=>{
      xx=clamp(Math.round(xx),0,proBaseCanvas.width-1);
      yy=clamp(Math.round(yy),0,proBaseCanvas.height-1);
      const i=(yy*proBaseCanvas.width+xx)*4;
      sr+=data[i];sg+=data[i+1];sb+=data[i+2];n++;
    };

    for(let xx=x;xx<x+w;xx+=4){
      sample(xx,y-pad); sample(xx,y+h+pad);
    }
    for(let yy=y;yy<y+h;yy+=4){
      sample(x-pad,yy); sample(x+w+pad,yy);
    }

    if(n){
      const rr=sr/n,gg=sg/n,bb=sb/n;
      bx.fillStyle=`rgb(${rr|0},${gg|0},${bb|0})`;
...
function drawZoneAt(ctx,z,norm){
  ...
  const amp=Number(proAmp.value);
  const dx=norm*amp*z.depth;

  // For pro cards, blocks move rigidly: no pixel warping.
  ctx.drawImage(
    src,
    z.x,z.y,z.w,z.h,
    z.x+dx,z.y,z.w,z.h
