...
async function cacheOnly(req){const c=await caches.match(req,{ignoreSearch:true});return c||new Response('Ressource absente du pack local HappyHolo.',{status:503});}
async function networkFirst(req){try{const r=await fetch(req,{cache:'no-store'});if(r&&(r.ok||r.type==='opaque')){const cache=await caches.open(RUNTIME_CACHE);cache.put(req,r.clone()).catch(()=>{});}return r;}catch(err){const c=await caches.match(req,{ignoreSearch:true});if(c)return c;throw err;}}
self.addEventListener('fetch',e=>{const req=e.request;if(req.method!=='GET')return;const url=new URL(req.url);if(req.mode==='navigate'){e.respondWith(happyHoloMode==='local'?caches.match(req,{ignoreSearch:true}).then(r=>r||caches.match('./relief3d-test-v31.html',{ignoreSearch:true})):networkFirst(req).catch(()=>caches.match('./relief3d-test-v31.html',{ignoreSearch:true})));return;}if(!cacheable(url)){if(happyHoloMode==='local')e.respondWith(new Response('Bloqué par MODE LOCAL HappyHolo.',{status:503}));return;}e.respondWith(happyHoloMode==='local'?cacheOnly(req):networkFirst(req));});
