...
self.addEventListener('fetch',e=>{
  ...
  // PixVerse reste volontairement accessible en MODE LOCAL : le coeur HappyHolo
  // utilise le cache local, mais une action IA demandée explicitement doit pouvoir
  // joindre nos endpoints serveur. Sans Internet, la requête échouera naturellement.
  if(isPixVerseApi(url)){
    e.respondWith(fetch(req,{cache:'no-store'}));
    return;
  }
  if(req.mode==='navigate'){
    e.respondWith(happyHoloMode==='local'?caches.match(req).then(r=>r||caches.match('./relief3d-test-v31.html')):networkFirst(req).catch(()=>caches.match('./relief3d-test-v31.html')));
    return;
  }
  if(!cacheable(url)){
