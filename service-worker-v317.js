const CACHE_VERSION = 'happyholo-offline-v1.43';
...
self.addEventListener('activate',e=>{
  e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('happyholo-offline-')&&k!==APP_CACHE&&k!==RUNTIME_CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
...
self.addEventListener('fetch',e=>{
  ...
  if(req.mode==='navigate'){e.respondWith(happyHoloMode==='local'?caches.match(req).then(r=>r||caches.match('./relief3d-test-v31.html')):networkFirst(req).catch(()=>caches.match('./relief3d-test-v31.html')));return;}
