...
function isPixVerseTestPage(url){return url.origin===self.location.origin&&url.pathname.endsWith('/happyholo-pixverse-actions-test.html');}
async function injectHappyHoloFixes(response,url){
  if(!response||!response.ok||(!isReliefPage(url)&&!isPixVerseTestPage(url)))return response;
  try{
    const html=await response.clone().text();let scripts='';
    if(isReliefPage(url)){
      if(!html.includes('background-multiselect-engine.js'))scripts+='<script src="./background-multiselect-engine.js?v=20260830a"></script>';
      if(!html.includes('background-object-editor-ipad-fix.js'))scripts+='<script src="./background-object-editor-ipad-fix.js?v=20260830a"></script>';
      if(!html.includes('pixverse-text-separation.js'))scripts+='<script src="./pixverse-text-separation.js?v=20260830a"></script>';
    }
    if(isPixVerseTestPage(url)&&!html.includes('pixverse-text-bridge.js'))scripts+='<script src="./pixverse-text-bridge.js?v=20260830a"></script>';
...
self.addEventListener('fetch',e=>{
  ...
  if(req.mode==='navigate'){
    e.respondWith((async()=>{let r;if(happyHoloMode==='local')r=await caches.match(req)||await caches.match('./relief3d-test-v31.html');else r=await networkFirst(req).catch(()=>caches.match('./relief3d-test-v31.html'));return injectHappyHoloFixes(r,url);})());return;
  }
