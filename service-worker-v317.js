const CACHE_VERSION = 'happyholo-offline-v1.44';
...
const APP_SHELL = [
  ...
  './mask-visibility-v318.js',
  './selection-controls-v336.js',
  './custom-background-v338.js',
  './composition-advanced-v350.js',
  './preview-sync-v355.js',
  './text-layer-v337.js',
  './v311-monotonic-patch.js',
  './support-preview-v316.js',
  './support-placement-sync-v374.js',
...
self.addEventListener('fetch',e=>{const req=e.request;if(req.method!=='GET')return;const url=new URL(req.url);if(req.mode==='navigate'){e.respondWith(happyHoloMode==='local'?caches.match(req).then(r=>r||caches.match('./relief3d-test-v31.html')):networkFirst(req).catch(()=>caches.match('./relief3d-test-v31.html')));return;}if(!cacheable(url)){if(happyHoloMode==='local')e.respondWith(new Response('Bloqué par MODE LOCAL HappyHolo.',{status:503}));return;}e.respondWith(happyHoloMode==='local'?cacheOnly(req):networkFirst(req));});
