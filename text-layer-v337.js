/* HappyHolo V3.4.4 — texte en relief compatible carte */
(() => {
  'use strict';
  ...
  function draw(ctx,norm=0,box=null){
    ...
    ctx.shadowBlur=Math.max(1,fontSize*.07);
    ctx.shadowOffsetY=Math.max(1,fontSize*.03);
  ...
  card.innerHTML=`
    <div style="display:flex;justify-content:space-between;gap:12px;align-items:center">
      <div><h2 style="margin:0 0 4px">Texte en relief / carte</h2><div class="small">Jusqu’à 4 lignes. Repris dans l’aperçu principal, le simulateur et les 9 vues.</div></div>
    ...
    <div id="textTools" style="display:grid;grid-template-columns:minmax(220px,1.1fr) minmax(220px,1fr);gap:16px;margin-top:14px;opacity:.48">
      ...
      <div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
          <button id="textCenter" type="button" class="secondary">Recentrer</button>
          <button id="textCardPreset" type="button" class="secondary">Preset carte</button>
          <span class="small" style="align-self:center">Déplace le texte directement sur la grande image.</span>
        </div>
      </div>
    </div>`;
  ...
  size.value=state.size??8;orientation.value=state.orientation??0;depth.value=state.depth??55;rotation.value=state.rotation3D??14;
