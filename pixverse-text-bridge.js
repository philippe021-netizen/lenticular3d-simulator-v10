...
async function exportPixVerseWithText(e){
  ...
  if(!result.qualityGate?.passed){if(status){status.textContent='Export bloqué : retour brutal à la pose initiale confirmé.';status.style.background='#ffe9e7';status.style.color='#8b1f17';}btn.disabled=false;return;}
