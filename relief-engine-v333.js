...
function drawReliefIntoSupport(norm){
  ...
  const type=supportType.value;
  const source=supportSource.value;
  ...
  if(source==='pro'){
    ...
    // Render current pro-card multilayer preview into offscreen canvas.
...
function syncSupportOptions(){
  ...
  if(source==='pro'){
    supportType.value='card';
    supportType.querySelector('option[value="medal"]').disabled=true;
    supportStatus.textContent = proSourceCanvas
      ? 'Carte pro 3D prête à être affichée sur le support carte.'
      : 'Crée d’abord la Carte pro 3D dans l’onglet correspondant.';
  }else{
    supportType.querySelector('option[value="medal"]').disabled=false;
    supportStatus.textContent = reliefReady
      ? 'Relief photo prêt à être affiché sur support.'
      : 'Crée d’abord le relief 3D dans le premier onglet.';
  }

  updateSupportShape();

  if((source==='pro' && proSourceCanvas) || (source==='relief' && reliefReady)){
    renderSupportFrame(0);
    product.style.transform='none';
  }
}
