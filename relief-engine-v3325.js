...
tabs.forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    if(target==='support') setTimeout(()=>{ syncSupportOptions(); syncSupportChoiceButtons(); },0);
  });
});

const supportViewMode=$('#supportViewMode');
const supportMaskMode=$('#supportMaskMode');
const supportMaskLabel=$('#supportMaskLabel');
const supportSource=$('#supportSource');
let lastReadySource=null;
const supportType=$('#supportType');
const supportChoiceButtons=[...document.querySelectorAll('.support-choice')];
...
const supportStage=$('#supportStage');
const supportCanvas=$('#supportCanvas');
const cardPortraitTemplate=$('#cardPortraitTemplate');
const cardLandscapeTemplate=$('#cardLandscapeTemplate');
const keychainVerticalTemplate=$('#keychainVerticalTemplate');
const keychainHorizontalTemplate=$('#keychainHorizontalTemplate');
const medallionTemplate=$('#medallionTemplate');
...
function cardOrientationScores(img){
  if(!img) return null;
  ...
  const evaluate=(targetRatio)=>{
    const sourceRatio=w/h;

    let retention=1;
    if(sourceRatio>targetRatio){
      retention=(h*targetRatio)/w;
    }else{
      retention=(w/targetRatio)/h;
    }
    ...
    const extension=Math.max(0,(targetW*targetH-w*h)/(w*h));
    const score=extension*1.25 + (1-retention)*0.75;

    return {retention,extension,score};
  };

  const landscape=evaluate(landscapeRatio);
  const portrait=evaluate(portraitRatio);

  return {
    landscape,
    portrait,
    best: portrait.score < landscape.score ? 'portrait' : 'landscape'
  };
}

function automaticCardOrientation(){
  const scores=cardOrientationScores(sourceImg || originalSourceImg);
  return scores?.best || 'landscape';
}

function currentCardOrientation(){ return 'portrait'; }
function syncSupportOrientationLabel(){}

function applySupportViewMode(){
  const mode=supportViewMode.value;
  const mask=supportMaskMode.value;

  product.classList.remove('simple-product','safe-zone');

  if(mode==='simple'){
    product.classList.add('simple-product');

    if(mask==='safe'){
      product.classList.add('safe-zone');
    }

    // Neutral technical stage, no fake marketing card frame.
    if(supportStage) supportStage.style.background=
      'radial-gradient(circle at 50% 40%,#f7f7f7 0,#ececec 58%,#e2e2e2 100%)';

    // Keep canvas shape only.
  ...
  if(supportMaskLabel) supportMaskLabel.style.display=mode==='simple' ? '' : 'none';
  if(supportMaskMode) supportMaskMode.style.display=mode==='simple' ? '' : 'none';
}

function mountSupportCanvas(target){
  if(supportCanvas.parentElement!==target){
    target.appendChild(supportCanvas);
  }
}
...
function updateSupportShape(){
  ...
  cardPortraitTemplate.hidden=true;
  cardLandscapeTemplate.hidden=true;
  keychainVerticalTemplate.hidden=true;
  keychainHorizontalTemplate.hidden=true;
  medallionTemplate.hidden=true;

  product.className='product support-object';
  product.style.aspectRatio='';
  product.style.marginTop='';
  product.style.marginLeft='';

  if(type==='card-portrait'){
    product.classList.add('card-portrait-object');
    cardPortraitTemplate.hidden=false;
    mountSupportCanvas(cardPortraitTemplate.querySelector('.card-window-portrait'));
    supportCanvas.width=756;
    supportCanvas.height=1200;

  }else if(type==='card-landscape'){
    product.classList.add('card-landscape-object');
    cardLandscapeTemplate.hidden=false;
    mountSupportCanvas(cardLandscapeTemplate.querySelector('.card-window-landscape'));
    supportCanvas.width=1200;
    supportCanvas.height=756;

  }else if(type==='keychain-vertical'){
    product.classList.add('keychain-vertical-object');
    keychainVerticalTemplate.hidden=false;
    mountSupportCanvas(keychainVerticalTemplate.querySelector('.kv-window'));
    supportCanvas.width=800;
    supportCanvas.height=1200;

  }else if(type==='keychain-horizontal'){
    product.classList.add('keychain-horizontal-object');
    keychainHorizontalTemplate.hidden=false;
    mountSupportCanvas(keychainHorizontalTemplate.querySelector('.kh-window'));
    supportCanvas.width=1200;
    supportCanvas.height=800;

  }else{
    product.classList.add('medallion-object');
    medallionTemplate.hidden=false;
    mountSupportCanvas(medallionTemplate.querySelector('.medallion-window'));
    supportCanvas.width=900;
    supportCanvas.height=900;
  }

  applySupportViewMode();
}
...
function drawReliefIntoSupport(norm){
  ...
  if(source==='pro'){
    ...
    // A vertical card is a rotated presentation of the same business-card artwork.
    if(type==='card-portrait' && temp.width>temp.height){
      const rotated=document.createElement('canvas');
      rotated.width=temp.height;
      rotated.height=temp.width;
      const rx=rotated.getContext('2d');
      rx.translate(rotated.width/2,rotated.height/2);
      rx.rotate(Math.PI/2);
      rx.drawImage(temp,-temp.width/2,-temp.height/2);
      temp=rotated;
    }
    ...
    if(supportFit.value==='preserve' && type!=='medal'){
  ...
  if(type!=='medal'){
    ...
    if(fitMode==='preserve' && source==='relief'){
      ...
      let baseScale;
      // Carte pro: conserve toujours la totalité de la carte.
      // Cela évite de couper logo, texte ou coordonnées.
      if(source==='pro' || fitMode==='contain'){
...
function renderSupportFrame(norm){
  ...
  if(supportViewMode.value==='simple'){
    product.style.background='transparent';
  }
  if(!ok) return;

  const maxA=Math.min(8,Number(supportAngle.value));
  const rotY=norm*maxA;

  // The parent contains ring + link + frame + image:
  // the whole physical object rotates as one unit.
  product.style.transform=
    `perspective(1300px) rotateY(${rotY}deg)`;
}
...
supportPlay.addEventListener('click',()=>{
  ...
  if(source==='pro'){
    supportStatus.textContent = supportType.value==='card-portrait'
      ? 'Aperçu Carte pro 3D verticale en cours.'
      : 'Aperçu Carte pro 3D horizontale en cours.';
  }else if(supportType.value==='card-portrait'){
    supportStatus.textContent='Aperçu carte verticale en cours.';
  }else if(supportType.value==='card-landscape'){
    supportStatus.textContent='Aperçu carte horizontale en cours.';
  }else if(supportType.value==='keychain-vertical'){
    supportStatus.textContent='Aperçu porte-clé vertical en cours.';
  }else if(supportType.value==='keychain-horizontal'){
    supportStatus.textContent='Aperçu porte-clé horizontal en cours.';
  }else{
...
function syncSupportOptions(){
  ...
  if(source==='pro'){
    // A business card / CB must be displayed as a card, never inside a keychain.
    if(!['card-portrait','card-landscape'].includes(supportType.value)){
      supportType.value='card-landscape';
    }

    supportStatus.textContent = proSourceCanvas
      ? 'Carte pro 3D détectée — support carte horizontal sélectionné.'
      : 'Crée d’abord la Carte pro 3D.';

  }else if(source==='relief'){
    supportStatus.textContent = reliefReady
      ? 'Relief photo détecté — choisis carte, porte-clé ou médaillon.'
...
function syncSupportChoiceButtons(){
  supportChoiceButtons.forEach(btn=>{
    const on=btn.dataset.support===supportType.value;
...
function chooseSupport(type){
  if(!['card-portrait','card-landscape','keychain-vertical','keychain-horizontal','medal'].includes(type)) return;

  // Stop the current preview first.
  supportRunning=false;
  cancelAnimationFrame(supportRAF);
  supportRAF=0;
  supportStartTime=0;
  ...
  // Keep the hidden select only as shared state.
  supportType.value=type;

  // Directly rebuild the selected physical support.
  updateSupportShape();
  syncSupportChoiceButtons();
  ...
  if(type==='card-portrait'){
    supportStatus.textContent='Carte verticale sélectionnée.';
  }else if(type==='card-landscape'){
    supportStatus.textContent='Carte horizontale sélectionnée.';
  }else if(type==='keychain-vertical'){
    supportStatus.textContent='Porte-clé vertical sélectionné.';
  }else if(type==='keychain-horizontal'){
    supportStatus.textContent='Porte-clé horizontal sélectionné.';
  }else{
    supportStatus.textContent='Médaillon rond sélectionné.';
  }
}
...
supportChoiceButtons.forEach(btn=>{
  btn.addEventListener('click',(ev)=>{
    ev.preventDefault();
    ev.stopPropagation();
    chooseSupport(btn.dataset.support);
  });
});
...
function refreshSupportViewMode(){
  supportRunning=false;
  cancelAnimationFrame(supportRAF);
  supportRAF=0;
  supportStartTime=0;
  ...
  supportStatus.textContent=
    supportViewMode.value==='simple'
      ? 'Simulation simple prête — aperçu lenticulaire sans mockup parasite.'
      : 'Présentation produit prête.';
}

supportViewMode.addEventListener('change',refreshSupportViewMode);
supportMaskMode.addEventListener('change',refreshSupportViewMode);

const marketingBadge=document.querySelector('.support-stage .badge, .support-stage [class*="badge"]');
function syncMarketingBadge(){
  if(!marketingBadge) return;
  marketingBadge.style.display=supportViewMode.value==='simple' ? 'none' : '';
}
supportViewMode.addEventListener('change',syncMarketingBadge);
syncMarketingBadge();
