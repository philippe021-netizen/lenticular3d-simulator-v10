...
let livePanel=null,liveCanvas=null,liveCtx=null,liveRAF=0,liveRunning=true,liveStart=0;
...
function ensureLiveSimulator(state){
  if(livePanel?.isConnected)return;const grid=document.querySelector('.grid'),left=grid?.firstElementChild;if(!left)return;livePanel=document.createElement('section');livePanel.id='hhLiveCardSimulator';livePanel.className='card';Object.assign(livePanel.style,{position:'sticky',top:'76px',zIndex:'8',border:'2px solid #17652c',padding:'12px',background:'#fff',boxShadow:'0 8px 28px rgba(0,0,0,.10)'});livePanel.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;gap:8px"><div><div style="font-size:11px;font-weight:900;color:#17652c;text-transform:uppercase">Simulation temps réel</div><b style="font-size:15px">Effet lenticulaire final</b></div><button id="hhLiveToggle" type="button" style="width:auto;margin:0;padding:8px 10px;background:#111">Pause</button></div><canvas id="hhLiveCanvas" style="display:block;width:100%;aspect-ratio:1.5858/1;margin-top:10px;border-radius:12px;background:#111"></canvas><div style="font-size:11px;color:#666;line-height:1.35;margin-top:7px">Le fond parcourt automatiquement les 9 vues. Les textes et logos montrent leur profondeur réelle pendant que tu modifies les réglages.</div>`;left.appendChild(livePanel);liveCanvas=livePanel.querySelector('#hhLiveCanvas');liveCtx=liveCanvas.getContext('2d');livePanel.querySelector('#hhLiveToggle').onclick=e=>{liveRunning=!liveRunning;e.currentTarget.textContent=liveRunning?'Pause':'Lecture';if(liveRunning)liveStart=0;drawLiveSimulator(state,performance.now());};const textCard=document.getElementById('textCard');const syncVisibility=()=>{const active=textCard&&!textCard.classList.contains('hidden');livePanel.style.display=active?'block':'none';if(active)drawLiveSimulator(state,performance.now());};if(textCard)new MutationObserver(syncVisibility).observe(textCard,{attributes:true,attributeFilter:['class']});syncVisibility();cancelAnimationFrame(liveRAF);liveRAF=requestAnimationFrame(liveLoop);
}
