/* HappyHolo — éditeur de masque V3.1.6 iPad
   Correctifs UX :
   - suppression des sélections/callouts parasites du navigateur
   - plage de pinceau étendue pour remplissage large et détails très fins
*/
(() => {
  'use strict';
  ...
  function buildUI(){
    ...
    ['contextmenu','selectstart','dragstart'].forEach(type=>{
      modal.addEventListener(type,e=>e.preventDefault());
    });
    ...
    work=el('div',{style:{
      ...
      background:'#111',userSelect:'none',WebkitUserSelect:'none',WebkitTouchCallout:'none',WebkitTapHighlightColor:'transparent'
    }},body);
    ...
    baseCanvas=el('canvas',{style:{...canvasStyle,pointerEvents:'none'}},work);
    baseCtx=baseCanvas.getContext('2d');

    editCanvas=el('canvas',{style:{...canvasStyle,touchAction:'none'}},work);
    ...
    loupe=el('canvas',{
      ...
      style:{width:'100%',height:'100%',display:'block',aspectRatio:'auto',borderRadius:'0',background:'#000'}
    },lw);
  ...
  function drawBase(){
    ...
    const r=rect();
    const sx=baseCanvas.width/Math.max(1,r.width);
    const sy=baseCanvas.height/Math.max(1,r.height);
  ...
  function drawOverlay(){
    ...
    const r=rect();
    const sx=editCanvas.width/Math.max(1,r.width);
    const sy=editCanvas.height/Math.max(1,r.height);
  ...
  async function validateEditor(){
    ...
    const blob=await new Promise((res,rej)=>
      out.toBlob(
        b=>b?res(b):rej(new Error('PNG masque impossible')),
        'image/png'
      )
    );
  ...
  window.localRemoveBackground=async function(file){
    ...
    if(!corrected){
      throw new Error('Correction du masque annulée.');
    }

    return corrected;
  };

  console.log('[HAPPYHOLO] éditeur masque V3.1.6 pinceau/callout actif');
})();
