...
function refreshSupportCropLabels(){
...
[supportFit,supportZoom,supportX,supportY].forEach(el=>{
  el.addEventListener('input',refreshSupportCropLabels);
  el.addEventListener('change',refreshSupportCropLabels);
});
...
function updateSupportShape(){
  ...
  if(type==='card'){
    ...
    const offX=Number(supportX.value)/100;
    const offY=Number(supportY.value)/100;
    ...
    // Manual offsets are percentages of the remaining visible dimension.
    const dx=(W-dw)/2 + offX*W*.5;
    const dy=(H-dh)/2 + offY*H*.5;
...
supportPlay.addEventListener('click',()=>{
  ...
  supportStatus.textContent =
    source==='pro'
      ? 'Aperçu de la Carte pro 3D sur support en cours.'
      : (supportType.value==='card'
          ? 'Aperçu carte en cours.'
...
supportType.addEventListener('change',()=>{
  updateSupportShape();
  if(
    (supportSource.value==='pro' && proSourceCanvas) ||
    (supportSource.value==='relief' && reliefReady)
  ){
    renderSupportFrame(0);
    product.style.transform='none';
  }
...
// V3.3.5 — cadrage support defaults
if(supportFit) supportFit.value='contain';
if(supportZoom) supportZoom.value='100';
if(supportX) supportX.value='0';
if(supportY) supportY.value='0';
if(typeof refreshSupportCropLabels==='function') refreshSupportCropLabels();
